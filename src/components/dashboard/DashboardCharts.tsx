...
export function DashboardCharts({records}:{records:FoodQualityRecord[]}){
  ...
  const standard=qualityStandards[trendProduct];
  const phEdge=standard?(standard.ph.max-standard.ph.min)*0.1:0;
  const waterWarningStart=standard?standard.waterActivity.max*0.9:0;
  ...
  return <div className="grid gap-5 xl:grid-cols-2">
    ...
    <Box title="pH Trend" subtitle="One product at a time, with demonstration specification and warning zones">{productSelector}{wrap(<ResponsiveContainer><LineChart data={trend}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="test_date" tick={{fontSize:10}}/><YAxis domain={['auto','auto']}/><Tooltip/><Legend/>{standard&&<><ReferenceArea y1={standard.ph.min} y2={standard.ph.min+phEdge} fill="#f59e0b" fillOpacity={0.12}/><ReferenceArea y1={standard.ph.max-phEdge} y2={standard.ph.max} fill="#f59e0b" fillOpacity={0.12}/><ReferenceLine y={standard.ph.min} stroke="#e11d48" strokeDasharray="5 4" label={{value:'Min',position:'insideBottomLeft',fontSize:10}} ifOverflow="extendDomain"/><ReferenceLine y={standard.ph.max} stroke="#e11d48" strokeDasharray="5 4" label={{value:'Max',position:'insideTopLeft',fontSize:10}} ifOverflow="extendDomain"/></>}<Line connectNulls={false} dataKey="ph" name={`${trendProduct} pH`} stroke="#155483" strokeWidth={2} dot={{r:2}}/></LineChart></ResponsiveContainer>)}</Box>
    <Box title="Water Activity Trend" subtitle="Maximum specification and warning zone for the selected product">{productSelector}{wrap(<ResponsiveContainer><LineChart data={trend}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="test_date" tick={{fontSize:10}}/><YAxis domain={['auto','auto']}/><Tooltip/><Legend/>{standard&&<><ReferenceArea y1={waterWarningStart} y2={standard.waterActivity.max} fill="#f59e0b" fillOpacity={0.12}/><ReferenceLine y={standard.waterActivity.max} stroke="#e11d48" strokeDasharray="5 4" label={{value:'Maximum',position:'insideTopLeft',fontSize:10}} ifOverflow="extendDomain"/></>}<Line connectNulls={false} dataKey="water_activity" name={`${trendProduct} water activity`} stroke="#0891b2" strokeWidth={2} dot={{r:2}}/></LineChart></ResponsiveContainer>)}</Box>
